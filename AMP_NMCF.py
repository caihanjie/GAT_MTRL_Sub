...
class AMPNMCFEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _get_obs(self):
        ...
        self.OP_C0_norm = ActionNormalizer(action_space_low=self.C0_low, action_space_high=self.C0_high).reverse_action(self.op_results['C0']['c']) 
        self.OP_C1_norm = ActionNormalizer(action_space_low=self.C1_low, action_space_high=self.C1_high).reverse_action(self.op_results['C1']['c']) 
        ...
        if self.dcgain > 0 :
            ...
            if self.phase_margin < 45 or self.phase_margin > 90:
                ...
                if deviation <= 5: 
                    ...
                    self.phase_margin_score = - (deviation - 5) / 40  
